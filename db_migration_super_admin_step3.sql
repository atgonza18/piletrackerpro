-- =====================================================
-- SUPER ADMIN SYSTEM - STEP 3: Update existing RLS policies for super admin
-- =====================================================
-- CRITICAL STEP: This updates ALL existing RLS policies to allow super admin bypass.
-- We drop and recreate each policy to add the super admin check.
-- Pattern: is_current_user_super_admin() OR (existing condition)
--
-- IMPORTANT: Test this carefully! Have the rollback script ready.
...
CREATE POLICY "Project owners can insert new user associations"
  ...
  WITH CHECK (
    ...
    OR (
      ...
      OR user_projects.user_id = auth.uid()
    )
  );
...
-- =====================================================
-- PILE_LOOKUP_DATA TABLE POLICIES (update the ones from Step 2)
-- =====================================================

-- Drop the policies we just created in Step 2
...
COMMENT ON POLICY "Users can view projects they are part of" ON projects IS 'Updated to allow super admin access to all projects';
